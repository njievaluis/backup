...
// About Us and Voice Recognition functionality
document.addEventListener('DOMContentLoaded', function() {
    ...
    const aboutUsModal = document.getElementById('aboutUsContent');
    const closeButton = document.querySelector('.close-button');
    
    // Open the modal when the card is clicked
    aboutUsCard.addEventListener('click', function() {
        console.log('About Us card clicked!'); // Debug message
    ...
    fetch(GEOCODING_API_URL)
        ...
        .then(data => {
            if (data.length > 0) {
                ...
                alert(`City "${cityName}" not found. Please check the spelling and try again.`);
            }
        })
        .catch(error => {
